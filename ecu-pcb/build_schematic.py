...
NOTE_LINES = [
    ...
    "    - ADS1118-Q1 (U19), a real AEC-Q100 Grade 1 ADC that reads the",
    "      EGT thermocouple's raw millivolts directly over SPI, with cold-",
    "      junction compensation and ITS-90 K-type linearisation moved into",
    "      firmware. SUPERSEDES an earlier AD8495ARZ thermocouple amplifier",
    "      (right FUNCTION, but no AEC-Q100 statement anywhere in Analog",
    "      Devices' own datasheet, and no automotive-qualified dedicated",
    "      thermocouple amplifier IC exists to swap it for - MAX31855,",
    "      MAX31856, AD8495, LTC2983 and MCP9600 were all checked). Real",
    "      compliance chosen over simpler hardware, the same tradeoff the",
    "      sibling thermo-pcb project hit and resolved the same way first.",
    ...
    "      active LOW). AEC-Q100 Grade 1 qualification CONFIRMED in a later",
    "      pass, from NXP's own Rev.13 datasheet revision history ('Added",
    "      AEC-Q100 grade 1 qualified') - absent from the Rev.10 text this",
    "      pass first worked from.",
    "    Real catches worth recording: (1) EGT's front end is TI's own Fig",
    "    50 two-channel thermocouple topology (ADS1118 datasheet SBAS457F)",
    "    - R78/R91 (1.00M) bias the floating thermocouple into range AND",
    "    give free open-lead detection, R92/R93 (499R) are the per-leg",
    "    anti-aliasing series resistors, C88/C89/C90 the differential/",
    "    common-mode filter caps, and R77 (10k) is a CS pull-up so the ADC",
    "    doesn't latch a garbage config write while the MCU's GPIO floats",
    "    at power-up;",
